...
pub struct ImageExporter<F: Fn(i64) -> PathBuf> {
    path_generator: F,
    configured: Option<ConfiguredProperties>,
}

impl<F: Fn(i64) -> PathBuf> ImageExporter<F> {
    pub fn new(path_generator: F) -> Self {
        Self {
            path_generator,
            configured: None,
        }
...
impl<F: Fn(i64) -> PathBuf> OutputHandler for ImageExporter<F> {
    ...
    fn publish_frame(
        ...
        match receiver.recv().unwrap() {
            Ok(()) => {
                ...
                let path = (self.path_generator)(frame);
